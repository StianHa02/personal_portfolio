...
/**
 * Playwright's mark, hand-rolled because no icon pack we use carries it:
 * simple-icons has no Playwright entry, and `react-icons/si` is generated from
 * simple-icons, so `SiPlaywright` does not exist at any version. This is the
 * monochrome single-path variant from Devicon (MIT), which inherits
 * `currentColor` and so tints through `mute()` exactly like the Si icons
 * beside it in the skills grid.
 *
 * Signature matches react-icons' `IconType` so it drops into `Skill.icon`
 * without a special case.
 */
export function PlaywrightIcon(props: IconBaseProps) {
    return (
        <svg
            // Tightened to the artwork's own bounding box. Devicon draws inside
            // padding that the simple-icons glyphs beside it do not have, so at the
            // nominal 0 0 128 128 this painted ~26px wide where Git, Vercel and
            // Docker paint 30px, and read as undersized. Cropping the padding makes
            // it match at any font size rather than needing a per-use scale.
            viewBox="8.8 9.5 108.9 108.9"
            fill="currentColor"
            height="1em"
            width="1em"
            aria-hidden="true"
