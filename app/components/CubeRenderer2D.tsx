...
import {
    type Vec3, type FaceKey, FACE_COLORS, DARK,
    rx, ry, rot, N6, cubeScale,
    buildSolved, buildPlayback, PB_SCRAMBLE, parseMoves, applyFaceMoves, resolveScroll,
    invertSequence, simplify,
} from "./cube/state";

// The no-WebGL fallback has no interaction, so one scramble and the exact way back
// out of it — built once, synchronously, no solver needed.
const FALLBACK_PLAYBACK = (() => {
    const scramble = simplify(parseMoves(PB_SCRAMBLE));
    return buildPlayback(
        applyFaceMoves(buildSolved(), scramble),
        simplify(invertSequence(scramble)),
    );
})();
...
function drawScene(
    ...
    const { pieces, move: mv, t } = resolveScroll(FALLBACK_PLAYBACK, sp);

    const sc = cubeScale(W, H2);
    const cx = W / 2;
    const cy = H2 / 2;
    ...
    for (const {pos,stickers} of pieces) {
        ...
        for (let fi = 0; fi < 6; fi++) {
            ...
            // Bind the move itself, not just a boolean, so the rotations below stay typed.
            const turning = mv && Math.round(coord) === mv.layer ? mv : null;
            const la = turning ? turning.angle * t : 0;

            let n: Vec3 = [...N6[fi]] as Vec3;
            if (turning) n = rot(n, turning.axis, la);
            n = ry(n, autoRY); n = rx(n, autoRX);
            if (n[2] >= 0) continue;

            const fullVerts: Vec3[] = FACE_VERTS_FULL[fi].map(([ox,oy,oz]) => {
                let v: Vec3 = [pos[0] + ox, pos[1] + oy, pos[2] + oz];
                if (turning) v = rot(v, turning.axis, la);
...
export default function CubeRenderer2D({
                                         sp,
                                         opacity = 1,
                                         logoSrc,
